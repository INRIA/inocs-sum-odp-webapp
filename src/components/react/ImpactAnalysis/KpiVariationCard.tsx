...
export const KpiVariationCard: React.FC<KpiVariationCardProps> = ({
  ...
  if (size === "small") {
    return (
      <div
        className={`bg-gradient-to-br ${gradientClass} rounded-lg p-4 border-2 ${borderClass} shadow-sm hover:shadow-md transition-shadow`}
      >
  ...
  return (
    <div
      className={`bg-gradient-to-br ${gradientClass} rounded-xl p-6 border-2 ${borderClass} shadow-md hover:shadow-lg transition-shadow`}
    >
      <div className="flex flex-col gap-3">
        <h5 className="whitespace-pre-wrap">{title}</h5>
        {subtitle && <h6 className="whitespace-pre-wrap">{subtitle}</h6>}
        <div className="flex items-center gap-3">
          <h3 className="text-success" style={{ color }}>
            {value}
          </h3>
